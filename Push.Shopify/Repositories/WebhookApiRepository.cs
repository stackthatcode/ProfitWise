...
namespace Push.Shopify.Repositories
{
    ...
    public class WebhookApiRepository : IShopifyCredentialConsumer, IWebhookApiRepository
    {
        ...
        {
            var path = "/admin/api/2019-10/webhooks.json";
            var json = new
            {
                webhook = new
                {
        ...
        {
            var path = $"/admin/api/2019-10/webhooks/{request.Id}.json";
            var json = new
            {
                webhook = new
                {
        ...
        public Webhook Retrieve(string topic, string address)
        {
            ...
            var path = $"/admin/api/2019-10/webhooks.json?address={encodedAddress}&topic={encodedTopic}";
        ...
        {
            var encodedTopic = HttpUtility.UrlEncode(topic);
            var path = $"/admin/api/2019-10/webhooks.json?topic={encodedTopic}";
            var httpRequest = _requestFactory.HttpGet(ShopifyCredentials, path);
        ...
        {
            var path = $"/admin/api/2019-10/webhooks/{webHookId}.json";
